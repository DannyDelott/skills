---
name: code-review
description: Review the changes since a fixed point (commit, branch, tag, or merge-base) along two axes — Standards (does the code follow this repo's documented coding standards?) and Spec (does the code match what the originating issue/spec asked for?). Runs both reviews in parallel sub-agents and reports them side by side, then adds a reviewability profile and, when needed, a suggested semantic PR split. Use when the user wants to review a branch, a PR, work-in-progress changes, or asks to "review since X".
---
...
Run the Standards and Spec reviewers with GPT-5.6-Sol at high reasoning:
`model="gpt-5.6-sol"` and `reasoning_effort="high"`. Use the harness's
equivalent explicit model and reasoning fields when the field names differ. Do
not inherit the parent's model or silently fall back to another model. If the
runner cannot honor this override, stop before spawning the review and report
...
Anything in the repo that documents how code should be written, such as `CODING_STANDARDS.md`, `CONTRIBUTING.md`, or relevant files under `docs/` or `guides/` indexed from an `AGENTS.md`.
